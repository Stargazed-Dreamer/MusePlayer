...
> **✅ 战役已完结（2026-09-13）**：B1–B13、B9、S1、S2 全部完成并合入 master，
> 追加完成封装收紧 / Muse 导入补强 / 1.1.0 版本；CI 三平台测试 + 冒烟 + lint 全绿。
> 本文档保留作为**执行记录与经验存档**，新增任务不要再挂到本文件，
> 后续维护指引以 `AGENTS.md` 为准。
> **唯一遗留的条件任务**：S3（音频回调插值优化）——**仅在出现实际变速播放卡顿时才做**，
> 实施要点见第 5 节；无问题则永久搁置。
>
...
| 启动崩溃修复 + 边界元测试 | 2026-09-13：维护者真实启动发现 `main.py` 启动探针/延迟清理仍直访曲库字典（迁移时 grep 只覆盖 `app/`，漏了仓库根 `main.py`；异常在 QTimer 回调内进程不退出，CI 冒烟未拦截）。已改用访问器；新增 `tests/test_source_boundaries.py` 元测试固化"曲库字典只允许曲库家族访问"为 CI 防线。**教训：残留引用扫描必须覆盖全仓库（含根目录脚本）。**测试基线 **189 passed** |
...
**遗留人工验收（✅ 已于 2026-09-13 完成）**：

1. ~~B13 图标目检~~：维护者已逐状态核对 15 个图标（含播放/暂停、日夜主题、锁定/置顶等切换态），确认无差异；
2. ~~真机播放冒烟~~：维护者日常使用确认无"切歌后无声"或卡顿，S1 修复在实际音频设备上表现正常。
...
> **触发条件：仅当用户报告"变速播放（0.5x~4x 倍速）时出现可听卡顿/爆音"才启动实施；
> 1x 常速路径不走插值分支，不受此问题影响。无卡顿反馈则永久搁置，不得预防性重构。**

- **问题位置**：`core/core.py` `_audio_callback` 变速率路径（`abs(rate - 1.0) >= 1e-6` 分支），
  在持 `self._lock` 状态下对每个声道执行 `np.interp` 逐声道插值，与主线程/流式生产者线程争锁，
  理论上可能拉长回调持锁时长、诱发音频卡顿。
- **实施方向**（启动时按此设计，勿自行发挥）：
  1. 锁内仅做缓冲切片拷贝（`self._buffer[start:end]` 的浅复制到局部变量）；
  2. 释放锁后再执行 `np.interp` 插值与音量乘法；
  3. 插值结果写回 `outdata` 不需要锁（outdata 是 PortAudio 回调私有缓冲）；
  4. 注意游标推进 `self._frame_cursor` 仍须在锁内完成，或以局部变量计算后在锁内一次性提交。
- **验收**：`pytest tests/test_player_core.py` 全绿（现有 8 例覆盖回调相关语义）；
  真机以 0.5x / 1.5x / 2x 速率播放对比改造前后无可感知劣化。
- **状态**：未启动（截至 1.1.0 无卡顿反馈）。
